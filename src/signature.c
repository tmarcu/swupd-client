	...
	if (ret) {
		if (ret == X509_V_ERR_CERT_NOT_YET_VALID) {
			...
			if (stat("/usr/lib/os-release", &statt) != -1) {
				...
				strftime(timebuf, sizeof(timebuf), "%b %d, %Y", alttime);
				printf("Failure: Cerficiate not yet valid. Check the system clock. Should be at least %s\n",
						timebuf);
			} else {
				printf("Failure: Certificate not yet valid. Check system time\n");
			}
		} else {
